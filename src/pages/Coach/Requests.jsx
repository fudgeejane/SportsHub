  ...
  return (
    <section className="grid gap-4">
      ...
      <section className="rounded-3xl border border-slate-200 bg-white p-5 shadow-xl shadow-slate-900/5">
        <div className="mb-5">
          <h2 className="text-xl font-black text-slate-950">Join Requests</h2>
          <p className="mt-1 text-sm leading-6 text-slate-600">Review registration requests and roster outcomes.</p>
        </div>
              ...
              <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
                ...
                <div className="flex flex-wrap items-center gap-2">
                  ...
                  {request.status === JOIN_REQUEST_STATUS.PENDING ? (
                    <>
                      <button onClick={() => system.acceptJoinRequest(request)} className="rounded-xl bg-emerald-600 px-3 py-2 text-sm font-black text-white">
                        Accept
                      </button>
                      <button onClick={() => system.rejectJoinRequest(request.id)} className="rounded-xl border border-red-200 px-3 py-2 text-sm font-black text-red-600">
                        Reject
                      </button>
                    </>
                  ) : null}
